...
def write_results_to_file(filename, data):
    with open("txt_"+filename, 'a') as handle:
        hyper_para_str = ""
        for hyper_para in data:
            add_item = hyper_para+": "+ str(data[hyper_para])+", "
            hyper_para_str += add_item
        hyper_para_str+="\n"
        handle.write(hyper_para_str)
...
def get_processed_data(raw_sim_data,
    ...
    for patient_id in range(treatments.shape[0]):
        for timestep in range(treatments.shape[1]): # transfer to one-hot
...
def train_DR_optimal_model(dataset_train, dataset_val, hyperparams_file, model_name, model_folder,
