...
def fit_DRCRN_GRU_encoder(dataset_train, dataset_val, model_name, model_dir, hyperparams_file,
                    b_hyperparam_opt):
    ...
    if b_hyperparam_opt:
        ...
        for simulation in range(num_simulations):
            ...
            model = DR_CRN_GRU_Model(params, hyperparams)
    ...
    model = DR_CRN_GRU_Model(params, best_hyperparams)
    model.train(dataset_train, dataset_val, model_name, model_dir)#这里暂时取消训练，直接载入已训练模型



def DRCRN_GRU_encoder(pickle_map, models_dir,
    ...
    fit_DRCRN_GRU_encoder(dataset_train=training_processed, dataset_val=validation_processed,
